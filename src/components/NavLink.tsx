...
const GlobalStyle = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin:0; padding:0; }
    ...
    .hh-root { padding:10px 16px 8px; display:flex; flex-direction:column; gap:10px; }
    .hh-top { display:flex; align-items:center; justify-content:space-between; }
    .hh-loc { display:flex; flex-direction:column; }
    .hh-loc-top { display:flex; align-items:center; gap:4px; }
    .hh-name { font-size:16px; font-weight:800; color:${Z.dark}; line-height:1.2; }
    .hh-table { font-size:11px; font-weight:600; color:${Z.muted}; }
    .hh-right { display:flex; align-items:center; gap:8px; }
    .hh-icon-btn {
      width:38px; height:38px; border-radius:12px;
      display:flex; align-items:center; justify-content:center;
      background:${Z.surface}; border:1.5px solid ${Z.line}; transition:all .15s;
    }
    ...
    .hh-avatar {
      ...
      box-shadow:0 2px 8px rgba(226,55,68,.3); cursor:pointer;
    }
    .hh-search-row { display:flex; align-items:center; gap:8px; }
    .hh-search-wrap {
    ...
    .hh-search-input {
      ...
      font-size:13px; font-weight:500; color:${Z.dark}; outline:none;
    }
    .hh-search-input::placeholder { color:${Z.muted}; }
    .hh-mic-btn {
      width:38px; height:38px; border-radius:12px;
      display:flex; align-items:center; justify-content:center;
      flex-shrink:0; transition:all .15s; border:none; cursor:pointer;
    }
    .hh-mic-btn.idle { background:${Z.surface}; border:1.5px solid ${Z.line}; }
    .hh-mic-btn.recording { background:${Z.red}; border:1.5px solid ${Z.red}; animation:micPulse 1s infinite; }
    ...
    .profile-drawer {
      ...
      width:min(320px, 88vw); background:#fff; z-index:201;
      display:flex; flex-direction:column;
...
/* ═══════════════════════════════════════════════════════
   ENHANCED ADD BUTTON  (uses CartContext directly)
═══════════════════════════════════════════════════════ */
const AddButton = ({ item, small = false }: { item: MenuItem; small?: boolean }) => {
  ...
  const add = (e: React.MouseEvent) => {
    e.stopPropagation();
    addItem(
      { ...item, tableNumber: Number(tableNumber) || undefined },
      true,
      "HomeScreen"
    );
  };
...
const EnhancedHeader = ({ userName, tableNo, searchQuery, onSearch, onProfile }: EnhancedHeaderProps) => {
  ...
  return (
    <div className="hh-root">
      ...
      <div className="hh-search-row">
        ...
        <button className={`hh-mic-btn ${listening ? "recording" : "idle"} pk`} onClick={toggle}>
          {listening
...
/* ─────────────────────────────────────────────────────────
   PROFILE DRAWER
───────────────────────────────────────────────────────── */
const ProfileDrawer = ({ user, tableNo, onClose, navigate }: {
  user: any; tableNo: number | null; onClose: () => void; navigate: any;
}) => {
      ...
      <div className="profile-drawer">
        <div className="profile-header">
          ...
          <div style={{
            ...
            background: "rgba(255,255,255,.22)", border: "2.5px solid rgba(255,255,255,.4)",
          ...
          {isGuest ? (
            <button className="pk"
              ...
              style={{
                ...
                border: "none", cursor: "pointer", boxShadow: `0 4px 16px rgba(226,55,68,.3)`,
              }}>
...
const DineInBar = ({
  ...
  tableNo?: number; userName?: string;
  onCallWaiter: () => void; onGetBill: () => void; onOrderStatus: () => void;
}) => (
  <div className="dinebar fu0">
    <div style={{
      position: "relative", display: "flex", alignItems: "center",
      justifyContent: "space-between", padding: "12px 16px", gap: 8,
    }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <p style={{ fontSize: 15, fontWeight: 700, color: "#fff", lineHeight: 1.2 }}>
            {userName !== "Guest" && userName !== "guest@dineiq.com"
              ? userName.split(" ")[0] : "Guest"} · Table {tableNo}
          </p>
        </div>
      </div>
      <div style={{
        ...
        <span style={{
          width: 6, height: 6, borderRadius: "50%", background: Z.green,
          display: "block", flexShrink: 0, animation: "pulseGreen 1.7s infinite",
        }} />
...
const ActiveOrderBadge = ({ order }: { order: any }) => (
  <div style={{
    ...
    background: "rgba(27,166,114,.13)", border: "1px solid rgba(27,166,114,.28)",
    padding: "10px 14px",
  }}>
    <div style={{
      display: "flex", alignItems: "center",
      justifyContent: "space-between", marginBottom: 6, flexWrap: "wrap", gap: 4,
    }}>
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <div style={{ width: 40, height: 4, borderRadius: 99, background: Z.line, margin: "0 auto 18px" }} />
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <p style={{ fontSize: 11, color: Z.muted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
        ...
        <div style={{
          ...
          {[
            ...
            <div key={label} style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, color: green ? Z.green : Z.charcoal }}>{val}</span>
            </div>
          ))}
          <div style={{
            display: "flex", justifyContent: "space-between", alignItems: "center",
...
/* ─────────────────────────────────────────────────────────
   SECTION HEADER
───────────────────────────────────────────────────────── */
const SecHeader = ({ title, sub, badge, onAll }: {
...
const CategoryCarousel = ({ cats, selectedCategory, onPick }: {
  ...
  <div className="fu3">
    ...
    <div className="flex gap-3 px-4 overflow-x-auto hide-sb"
      ...
      <button className={`cat-carousel-card ${!selectedCategory ? "on" : ""}`} onClick={() => onPick("")}>
      ...
      {cats.map(cat => {
        ...
        return (
          <button key={cat.id} className={`cat-carousel-card ${sel ? "on" : ""}`} onClick={() => onPick(cat.name)}>
            <div className="flex items-center justify-center text-[26px]"
              style={{
...
const SmartComboCard = ({ item, isPersonalised }: { item: MenuItem; isPersonalised?: boolean }) => (
  <div className="combo-card pk">
    <div className="relative" style={{ height: 120 }}>
      ...
      <div className="absolute top-2 right-2"><VegDot isVeg={item.isVeg === true} /></div>
...
const ChefCard = ({ item }: { item: MenuItem }) => (
  <div className="chef-card pk">
    <div className="relative" style={{ height: 168 }}>
      ...
      <div className="absolute top-3 right-3"><VegDot isVeg={item.isVeg === true} /></div>
...
const MenuRow = ({ item }: { item: MenuItem }) => (
  <div className="menu-row pk">
    <div className="mt-1"><VegDot isVeg={item.isVeg === true} /></div>
    <div className="flex-1 min-w-0">
...
export default function HomeScreen() {
  ...
  /* ── Table number: URL param → context → localStorage → default ── */
  const tableNo = (() => {
  ...
  const handleGetBill    = () => { if (activeOrder) setShowBill(true); };
  const handleOrderStatus = () => {
    document.getElementById("active-order-strip")?.scrollIntoView({ behavior: "smooth", block: "center" });
  };
      ...
      try {
        ...
        if (menuData?.status === "success" && menuData?.menu_sections) {
          ...
          const mapToMenuItem = (item: any, category: string): MenuItem => {
            ...
            return {
              ...
              isCombo:     category === "Combos" || (item.Combo_Items?.length > 0) || Boolean(item.isCombo),
            };
          };
        ...
        if (GENERATE_AI_COMBOS) {
          ...
          try {
            ...
            if (aiData?.combos?.length > 0) setAiCombos(aiData.combos);
          } catch (err) {
  ...
  const personalisedCombos = useCallback((base: MenuItem[]): { items: MenuItem[]; isPersonalised: boolean } => {
    ...
    const historyCats = new Set(
      menuItems.filter(m => orderHistory.includes(m.id)).map(m => m.category.toUpperCase())
    );
  ...
  const handleHeroCta = () => {
    saveLog(user?.email || "Guest", "VIEW_MENU_BANNER_CLICK", "Hero CTA");
  ...
  const handleCheckout = () => {
    saveLog(user?.email || "Guest", "CART_CHECKOUT", `${totalItems} items · KSh ${totalPrice}`);
    navigate("/cart");
  };
    ...
    <>
      ...
      <div style={{ minHeight: "100vh", paddingBottom: 148, background: Z.surface }}>
        ...
        <div className="sticky top-0 z-50" style={{
          ...
          <EnhancedHeader
            userName={userName} tableNo={tableNo}
        ...
        <main>
          ...
          {!searchQuery && (
            <>
              {/* ① Hero Banner */}
              <HeroBanner onOrderNow={handleHeroCta} />

              {/* ② Dine-in bar */}
              <DineInBar
                tableNo={tableNo} userName={userName}
                  ...
                  <div id="smart-combos" style={{ background: Z.white, paddingTop: 20, paddingBottom: 20 }} className="scroll-mt-24">
                    <div className="flex items-start justify-between px-4 mb-3">
                      <div>
                        ...
                        <p className="sec-sub">
                          {isPersonalised ? "Curated from your order history" : "Popular combos — save more!"}
                        </p>
                      </div>
                      <button className="flex items-center gap-0.5 text-[13px] font-bold pk" style={{ color: Z.red }}>
                        See all <ChevronRight className="w-3.5 h-3.5" />
      ...
      {/* Cart Bar — uses CartContext directly */}
      <CartBar />

      {showWaiterMsg && <WaiterToast onDone={() => setShowWaiterMsg(false)} />}
      {showBill && activeOrder && (
        <BillModal order={activeOrder} tableNo={tableNo} onClose={() => setShowBill(false)} />
      )}
    </>
  );
}