...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  const [profile, setProfile] = useState<UserProfile>(() => {
    ...
    return saved
      ? JSON.parse(saved)
      : { id: "", name: "", phone: "", email: "", dateOfBirth: "" };
  });
  ...
  // ✅ Handle Table from QR or storage
  useEffect(() => {
  ...
  const login = (
    ...
    localStorage.setItem("dineiq_table_number", table); // ✅ persist
  ...
  const logout = () => {
    setTableNumber("");
    setGuestCount(1);
    setProfile({ id: "", name: "", phone: "", email: "", dateOfBirth: "" });
    setIsLoggedIn(false);
