...
export default function DishCard({ item, compact = false, source = "Menu" }: DishCardProps) {
  ...
  return (
    <div id={`menu-item-${item.id}`} className={`flex items-start gap-4 bg-white rounded-[20px] shadow-[0_1px_3px_rgba(0,0,0,0.02)] border border-white relative overflow-visible ${compact ? "p-3" : "p-4"}`}>

      {/* 1. Left Column: Image + (Optional) Button for Chef's Recommendations */}
      <div className="flex flex-col items-center gap-3 flex-shrink-0">
        ...
        {/* --- ADD BUTTON FOR CHEF'S RECOMMENDATIONS (Under Picture) --- */}
        {!compact && (
          <div className="w-[100px] shadow-md rounded-lg bg-white overflow-hidden">
            {quantity === 0 ? (
              <button
      ...
      {/* 3. Right Column: Add Button for Standard Categories (Bottom-Aligned) */}
      {compact && (
        <div className="flex-shrink-0 flex flex-col items-center justify-end min-w-[90px] self-stretch pl-2">
          <div className="w-full shadow-md rounded-lg bg-white overflow-hidden mb-1">
            {quantity === 0 ? (
              <button
                onClick={() => addItem(item, false, source)}
                className="w-full bg-[#FFF4F2] hover:bg-[#ffe5e5] text-[#E23744] border border-[#E23744]/20 font-extrabold text-xs h-8 rounded-lg uppercase tracking-wide flex items-center justify-center transition-colors"
          ...
          {quantity === 0 && (
            <p className="text-[8px] text-gray-400 font-semibold uppercase tracking-tighter">
              customisable
            </p>
          )}
        </div>
