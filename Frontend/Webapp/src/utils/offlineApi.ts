...
import { api } from "@/api";
import { smartFetch, dbSave, dbLoad, dbLoadAll, isOnline } from "@/db";
...
export const offlineApi = {
  ...
  signup:              (api as any).signup?.bind(api), // optional if not in api.ts
  checkUser:           (api as any).checkUser?.bind(api), // optional
  verifyOtp:           (api as any).verifyOtp?.bind(api), // optional
