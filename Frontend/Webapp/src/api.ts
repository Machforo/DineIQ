...
export const api = {
    ...
    savePreferences: async (email: string, preferences: any) => {
        try {
            const response = await fetchWithTimeout(`${API_BASE_URL}/save-preferences`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    fetchRecommendations: async (email: string, itemId: string) => {
        try {
            const response = await fetchWithTimeout(`${API_BASE_URL}/item-addons`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    async getPricingStrategy(email: string, cartItems: any[]) {
        try {
            const res = await fetchWithTimeout(`${API_BASE_URL}/pricing-strategy`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    async fetchUpsellItems() {
        try {
            const res = await fetchWithTimeout(`${API_BASE_URL}/upsell-items`);
            return await res.json();
        } catch (e) {
    ...
    async fetchCoupons() {
        try {
            const res = await fetchWithTimeout(`${API_BASE_URL}/coupons`);
            return await res.json();
        } catch (e) {
    ...
    async generateCombos(num: number = 3, email?: string) {
        try {
            const res = await fetchWithTimeout(`${API_BASE_URL}/generate-combos`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    async fetchOrderHistory(email: string) {
        try {
            const res = await fetchWithTimeout(`${API_BASE_URL}/order-history/${email}`);
            return await res.json();
        } catch (e) {
    ...
    async placeOrder(orderData: any) {
        try {
            const res = await fetchWithTimeout(`${API_BASE_URL}/place-order`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
